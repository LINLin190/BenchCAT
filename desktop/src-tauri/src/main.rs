...
fn preferred_window_size(
    ...
    const REFERENCE_SCREEN_WIDTH: f64 = 2560.0;
    const REFERENCE_SCREEN_HEIGHT: f64 = 1440.0;
    const REFERENCE_WINDOW_WIDTH: f64 = 1400.0;
    const REFERENCE_WINDOW_HEIGHT: f64 = 1050.0;
    const MIN_WIDTH: f64 = 960.0;
    const MIN_HEIGHT: f64 = 600.0;
    const EDGE_MARGIN: f64 = 32.0;

    let screen_scale =
        (screen_width / REFERENCE_SCREEN_WIDTH).min(screen_height / REFERENCE_SCREEN_HEIGHT);
    let preferred_width = REFERENCE_WINDOW_WIDTH * screen_scale;
    let preferred_height = REFERENCE_WINDOW_HEIGHT * screen_scale;
    ...
    tauri::LogicalSize::new(
        preferred_width.max(MIN_WIDTH).min(available_width),
        preferred_height.max(MIN_HEIGHT).min(available_height),
    )
}
...
fn configure_main_window(app: &tauri::App) {
    ...
    if let Some(monitor) = monitor {
        ...
        let logical_screen = monitor.size().to_logical::<f64>(scale_factor);
        let logical_work_area = work_area.size.to_logical::<f64>(scale_factor);
        let size = preferred_window_size(
            logical_screen.width,
            logical_screen.height,
            logical_work_area.width,
            logical_work_area.height,
        );
...
mod tests {
    ...
    fn preferred_window_size_uses_target_when_work_area_is_large_enough() {
        assert_eq!(
            preferred_window_size(2560.0, 1440.0, 2560.0, 1400.0),
            tauri::LogicalSize::new(1400.0, 1050.0)
        );
    }

    #[test]
    fn preferred_window_size_scales_for_a_1k_screen() {
        assert_eq!(
            preferred_window_size(1920.0, 1080.0, 1920.0, 1040.0),
            tauri::LogicalSize::new(1050.0, 787.5)
        );
    }
    ...
    fn preferred_window_size_never_exceeds_a_tiny_work_area() {
        assert_eq!(
            preferred_window_size(800.0, 480.0, 800.0, 480.0),
