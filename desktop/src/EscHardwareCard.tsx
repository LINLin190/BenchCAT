...
export function EscHardwareCard({ slave, profile, modelControl, modelNote }: { slave: SlaveInfo; profile: string; modelControl?: ReactNode; modelNote?: ReactNode }) {
  const disclosure = useDisclosure("bit 解析", "esc-hardware-details");
  const decoded = slave.esc_hardware ? decodeEscHardware(profile, slave.esc_hardware) : undefined;
  return (
    // Occupies one column. The model selector, the scanned bytes and the identification facts
    // all use the same body2 + mono treatment: they are the same level of information.
    <Card variant="outlined" className="ov-self-start">
    <CardContent className="ov-card-body">
      <CardHeading title="ESC 硬件信息" />
      ...
      {!decoded ? <Alert severity={slave.esc_hardware_error ? "warning" : "info"} sx={{ mt: 1 }}>
        ...
        <Box className="kv-compact" sx={{ mt: 1 }}>
          ...
          <Typography className="section-label">{decoded.family === "ET1100" ? "Product ID" : "Chip ID · ESC 型号"}</Typography>
